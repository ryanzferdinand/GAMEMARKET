...
function ReplyItem({ reply, threadId, onRefresh, depth = 0 }) {
  ...
  const vote = async (type) => {
    ...
    try { await api.post(`/forum/${threadId}/replies/${reply._id}/vote`, { type }); onRefresh() }
    ...
    <div className={depth > 0 ? 'ml-7 pl-4 border-l border-neutral-100 dark:border-neutral-800' : ''}>
      <div className="flex gap-3 py-4">
        ...
        <div className="flex flex-col items-center gap-0.5 pt-1 shrink-0">
          <button onClick={() => vote('up')} className={`p-0.5 rounded transition-colors ${myVote?.type === 'up' ? 'text-accent-600' : 'text-neutral-300 hover:text-accent-500'}`}>
            <MdThumbUp size={13} />
          </button>
          <span className={`text-2xs font-bold tabular-nums ${score > 0 ? 'text-emerald-500' : score < 0 ? 'text-red-500' : 'text-neutral-400'}`}>{score}</span>
          <button onClick={() => vote('down')} className={`p-0.5 rounded transition-colors ${myVote?.type === 'down' ? 'text-red-500' : 'text-neutral-300 hover:text-red-400'}`}>
            <MdThumbDown size={13} />
          </button>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {user && depth < 3 && (
            <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-1 mt-2 text-xs text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 transition-colors">
      ...
      {reply.replies?.map((r) => (
        <ReplyItem key={r._id} reply={r} threadId={threadId} onRefresh={onRefresh} depth={depth + 1} />
      ))}
    </div>
  )
}
...
export default function ForumDetailPage() {
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <button onClick={() => nav('/forum')} className="flex items-center gap-1.5 text-sm text-neutral-500 hover:text-neutral-800 dark:hover:text-neutral-200 transition-colors">
      ...
      <div className="card p-5 flex gap-4">
        <div className="flex flex-col items-center gap-1 pt-1 shrink-0">
          <button onClick={() => voteThread('up')} className={`p-1.5 rounded-xl transition-all ${myVote?.type === 'up' ? 'bg-accent-50 text-accent-600 dark:bg-accent-950/40 dark:text-accent-400' : 'text-neutral-300 hover:text-accent-500 hover:bg-accent-50 dark:hover:bg-accent-950/20'}`}>
            <MdThumbUp size={17} />
          </button>
          <span className={`text-sm font-bold tabular-nums ${score > 0 ? 'text-emerald-500' : score < 0 ? 'text-red-500' : 'text-neutral-400'}`}>{score}</span>
          <button onClick={() => voteThread('down')} className={`p-1.5 rounded-xl transition-all ${myVote?.type === 'down' ? 'bg-red-50 text-red-500 dark:bg-red-950/40' : 'text-neutral-300 hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20'}`}>
            <MdThumbDown size={17} />
          </button>
        </div>
        <div className="flex-1 min-w-0">
        ...
        {topLevel.length === 0 ? (
          ...
          <>
            <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
              {topLevel.map((r) => (
                <ReplyItem key={r._id} reply={r} threadId={id} onRefresh={() => load(1)} />
              ))}
            </div>
            {hasMore && (
