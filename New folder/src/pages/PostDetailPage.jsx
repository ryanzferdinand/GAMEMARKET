...
export default function PostDetailPage() {
  ...
  // ── Sticky mobile CTA (shown only when not owner & not sold, on small screens) ──
  const MobileCTA = !isOwner && !isSold && (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[var(--bg-elevated)] border-t border-[var(--border-soft)] px-4 py-3 flex gap-2"
      style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))' }}>
      {activeOrder ? (
        <Link to={`/orders/${activeOrder._id}`} className="btn-primary flex-1 justify-center gap-2" style={{ textDecoration: 'none' }}>
          <MdShoppingCart size={16} /> Lihat Pesanan
        </Link>
      ) : marketplace?.inTransaction ? (
        <div className="flex-1 text-center text-sm font-medium" style={{ color: '#ff9f0a', padding: '10px 0' }}>
          Sedang dalam transaksi
        </div>
      ) : needDeposit ? (
        <Link to="/wallet" className="btn-primary flex-1 justify-center" style={{ textDecoration: 'none' }}>
        ...
        <>
          <button onClick={() => { if (!user) return toast.error('Login untuk menghubungi penjual'); openChat(post.seller) }}
            className="btn-secondary" style={{ width: 44, padding: 0, justifyContent: 'center', flexShrink: 0 }}>
            <MdChatBubbleOutline size={18} />
          </button>
          <button onClick={handlePurchase} disabled={purchasing || !user} className="btn-primary flex-1 justify-center gap-2">
            <MdShoppingCart size={16} />
            {!user ? 'Login' : purchasing ? 'Memproses...' : `Beli · ${fmt(post.price)}`}
          </button>
        </>
      )}
    </div>
  ...
  return (
    <>
    {MobileCTA}
    <div className="space-y-5 animate-fade-up" style={{ paddingBottom: !isOwner && !isSold ? 80 : 0 }} >
      {/* Back */}
                  ...
                  {post.images?.length > 1 && (
                    <>
                      <button onClick={() => setImgIdx((i) => (i - 1 + post.images.length) % post.images.length)}
                        style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 32, height: 32, borderRadius: 9999, backgroundColor: 'rgba(210,210,215,0.64)', backdropFilter: 'blur(8px)', color: '#1d1d1f', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                        <MdChevronLeft size={20} />
                      </button>
                      <button onClick={() => setImgIdx((i) => (i + 1) % post.images.length)}
                        style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 32, height: 32, borderRadius: 9999, backgroundColor: 'rgba(210,210,215,0.64)', backdropFilter: 'blur(8px)', color: '#1d1d1f', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
