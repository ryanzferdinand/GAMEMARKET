...
export default function ChatPanel() {
  ...
  useEffect(() => {
    ...
    api.get(`/chat/history/${chatRecipient._id}`)
      .then(({ data }) => { setMessages(data); socket.emit('chat:read', { partnerId: chatRecipient._id }) })
  ...
  const Header = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0, minHeight: 52, cursor: minimized && !maximized ? 'pointer' : 'default', backgroundColor: 'var(--bg-elevated)' }}
              ...
              <div style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 4 }}>
                ...
                <div style={{ maxWidth: '78%', display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                  ...
                  {editingId === msg._id ? (
                    ...
                    <div style={{ padding: '8px 13px', borderRadius: isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px', fontSize: 15, lineHeight: 1.45, letterSpacing: '-0.224px', wordBreak: 'break-words', backgroundColor: isMe ? '#0066cc' : 'var(--bg-secondary)', color: isMe ? '#ffffff' : 'var(--text-primary)', opacity: msg.pending ? 0.6 : 1 }}>
                      {msg.content}
  ...
  return (
    <div style={{ position: 'fixed', bottom: 16, right: 16, zIndex: 50, width: 300, backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 18, boxShadow: 'var(--shadow-dropdown)', display: 'flex', flexDirection: 'column', overflow: 'hidden', height: minimized ? 52 : 420, transition: 'height 0.3s cubic-bezier(0.4,0,0.2,1)' }}>
      {Header}
      {!minimized && Body}
    </div>
  )
