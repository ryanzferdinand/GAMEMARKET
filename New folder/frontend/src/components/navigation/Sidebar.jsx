...
import {
  ...
  MdChatBubbleOutline, MdFavorite, MdStorefront,
} from 'react-icons/md'
      ...
      >
        <div style={{ flex: 1, padding: '10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {user && user.role !== 'buyer' && (
            <NavLink to="/seller/dashboard" style={{ textDecoration: 'none' }}>
              {({ isActive }) => <SideLink active={isActive} icon={MdStorefront} label="Dashboard Penjual" />}
            </NavLink>
          )}
          {user && (
