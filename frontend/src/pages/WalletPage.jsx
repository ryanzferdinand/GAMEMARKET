...
export default function WalletPage() {
  ...
  // Poll Midtrans payment status while pending — BUG-003: only poll when socket is disconnected
  useEffect(() => {
    if (!pendingPayment?.gatewayRef || pendingPayment.provider !== 'midtrans') return
    const socket = getSocket()
    if (socket?.connected) return // socket handles wallet:updated, no need to poll
