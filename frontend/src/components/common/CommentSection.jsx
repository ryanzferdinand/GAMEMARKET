...
// Returns a new votes array after toggling `type` for `userId`.
// - If user already voted the same type  → remove vote (toggle off)
// - If user voted the other type         → switch vote
// - If user hasn't voted                 → add vote
function applyOptimisticVote(votes = [], userId, type) {
  const existing = votes.find((v) => v.user === userId)
  if (existing?.type === type) {
    // toggle off
    return votes.filter((v) => v.user !== userId)
  }
  if (existing) {
    // switch type
    return votes.map((v) => v.user === userId ? { ...v, type } : v)
  }
  // new vote
  return [...votes, { user: userId, type }]
}

// ── Recursively apply a vote update to a comment tree ──────────────────────
function applyVoteToTree(comments, targetId, userId, type) {
  return comments.map((c) => {
    if (c._id === targetId) {
      return { ...c, votes: applyOptimisticVote(c.votes, userId, type) }
    }
    if (c.replies?.length) {
      return { ...c, replies: applyVoteToTree(c.replies, targetId, userId, type) }
    }
    return c
  })
}
...
function CommentItem({ comment, postId, onVote, onRefresh, depth = 0 }) {
  ...
  const handleVote = async (type) => {
    ...
    onVote(comment._id, user._id, type)
    try {
      await api.post(`/posts/${postId}/comments/${comment._id}/vote`, { type })
      // Server succeeded — optimistic state is already correct, no refresh needed
    } catch {
      toast.error('Gagal vote')
      // Rollback: revert the optimistic change by applying the inverse
      onVote(comment._id, user._id, type)
    } finally {
      setVoting(false)
    }
  ...
  return (
    <div className={depth > 0 ? 'ml-7 pl-4 border-l border-neutral-100 dark:border-neutral-800' : ''}>
      ...
      {comment.replies?.map((r) => (
        <CommentItem key={r._id} comment={r} postId={postId} onVote={onVote} onRefresh={onRefresh} depth={depth + 1} />
      ))}
    </div>
  )
}
...
/* ── CommentSection root ───────────────────────────────── */
export default function CommentSection({ postId, commentCount = 0, postStatus = 'approved' }) {
  const { user }         = useAuthStore()
  ...
  // Optimistic vote — mutates the comment tree in-place without a refetch
  const handleVote = useCallback((commentId, userId, type) => {
    setComments((prev) => applyVoteToTree(prev, commentId, userId, type))
  }, [])

  // Full refresh (only needed after new comment / reply)
  const refresh = useCallback(() => fetchComments(1), [fetchComments])
